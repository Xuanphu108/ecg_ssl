...
from .basic_conv1d import _conv1d
...
class CPCModel(nn.Module):
    ...
    def __init__(
        ...
        self.encoder = CPCEncoder(
            ...
            features = features, 
            bn = bn_encoder,
        ) if skip_encoder is False else None
        ...
        self.rnn = nn.LSTM(
            self.encoder_output_dim if skip_encoder is False else input_channels,
            n_hidden,
            num_layers = n_layers,
            batch_first = True) if lstm is True else nn.GRU(self.encoder.output_dim, n_hidden, num_layers = n_layers, batch_first = True)
        
        if (num_classes is None): # pretraining
            if (mlp): # additional hidden layer as in simclr
                self.proj = nn.Sequential(
                    nn.Linear(n_hidden, n_hidden), nn.ReLU(inplace = True), nn.Linear(n_hidden, self.encoder_output_dim, bias = bias_proj)
                )
            else:
