...
from models.masked.encoder.st_mem_vit_lw import ST_MEM_ViT_lw, TransformerBlock
...
class ST_MEM_lw(nn.Module):
    ...
    def forward_encoder(self, x, mask_ratio):
        ...
        for i in range(self.encoder.depth):
            ...
            layerout.append(self.encoder.norm(x))

        return layerout, mask, ids_restore  
    ...
    def forward_loss(self, series, mask_ratio = 0.75):
        ...
        elif self.loss_select == "mi": # Mutual Information (MI)
            # Normalize distributions
            pred = F.softmax(pred, dim = -1)
            target = F.softmax(target, dim = -1)
            # Compute entropies
            entropy_p = -torch.sum(pred * torch.log(pred + 1e-10), dim = -1)
            entropy_q = -torch.sum(target * torch.log(target + 1e-10), dim = -1)
            # Compute joint distribution
            joint = pred * target
            joint_entropy = -torch.sum(joint * torch.log(joint + 1e-10), dim = -1)
            # Compute Mutual Information
            mi = entropy_p + entropy_q - joint_entropy
            loss = -mi
