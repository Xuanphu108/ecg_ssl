...
class WindowAttention1D(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias = True, qk_scale = None, attn_drop = 0., proj_drop = 0.):
        ...
        relative_coords = relative_coords.permute(1, 2, 0).contiguous()  # Wl, Wl, 2
    ...
    def forward(self, x, mask = None):
        ...
        relative_position_bias = self.relative_position_bias_table[self.relative_position_index.view(-1)].view(self.window_size, self.window_size, -1)  # Wl, Wl, nH
...
class SwinTransformerBlock1D(nn.Module):
    ...
    def forward_part1(self, x, mask_matrix):
        ...
        shifted_x = window_reverse(attn_windows, window_size, Lp)  # B D' H' W' C
...
class PatchMerging(nn.Module):
    ...
    def __init__(self, dim, encoder, norm_layer = nn.LayerNorm):
        super().__init__()
        self.dim = dim
...
class BasicLayer(nn.Module):
    ...
    def __init__(self,
        ...
        if self.downsample is not None:
            self.downsample = downsample(dim = dim, encoder = self.encoder, norm_layer = norm_layer)
