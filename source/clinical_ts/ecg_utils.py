__all__ = ['get_available_channels', 'channel_stoi_default', 'resample_data', 'get_filename_out', 'prepare_data_ptb_xl', 'filter_ptb_xl', 'prepare_data_cinc',  
           'prepare_data_zheng', 'prepare_data_ribeiro_test', 'prepare_data_code_15', 'prepare_data_code_15_chagas', 'prepare_data_leuven2024']
...
channel_stoi_default = {
    "i": 0, "ii": 1, "v1": 2, "v2": 3, "v3": 4, "v4": 5, "v5": 6, "v6": 7, "iii": 8, "avr": 9, "avl": 10, "avf": 11, "vx": 12, "vy": 13, "vz": 14
}
...
def prepare_data_cinc(
        data_path, datasets = ["ICBEB2018", "ICBEB2018_2", "INCART", "PTB", "PTB-XL", "Georgia"], 
        # data_path, datasets = ["ICBEB2018", "ICBEB2018_2", "INCART", "Georgia"],
        target_fs = 100, 
        strat_folds = 10, 
        channels = 8, 
        channel_stoi = channel_stoi_default, 
...
def prepare_data_zheng(
    ...
    if (recreate_data is True):
        ...
        # stratified_ids = stratify(list(df["label_rhythm_txt"]), lbl_itos["rhythm"], [1./strat_folds] * strat_folds)
        df["strat_fold"] = -1
    ...
    if (recreate_data is True):
        ...
        def reformat_predictions(df, colname_txt = "label_txt", colname_num = "label", lbl_itos = ["1AVB", "RBBB", "LBBB", "SBRAD", "AFIB", "STACH"]):
            ...
            df[colname_txt] = df.apply(
                lambda row: ("1AVB " if row["1dAVb"] else "") + ("RBBB " if row["RBBB"] else "") + \
                ("LBBB " if row["LBBB"] else "") + ("SBRAD " if row["SB"] else "") + ("AFIB " if row["AF"] else "") + ("STACH " if row["ST"] else ""),
                axis=1,
            )
...
def prepare_data_code_15(
    ...
    if (recreate_data is True):
        ecg_lbl_itos = ["1AVB", "RBBB", "LBBB", "SB", "ST", "AF"]
        channel_labels = ["i", "ii", "iii", "avr", "avl", "avf", "v1", "v2", "v3", "v4", "v5", "v6"]
        fs = 400.

        # prepare df
        exams_df = pd.read_csv(data_path/"exams.csv")
        exams_df["sex"] = exams_df["is_male"].apply(lambda x: "male" if x else "female")
        exams_df.drop(["is_male"], axis = 1, inplace = True)

        def reformat_predictions_code_15(
                df, 
                ecg_colname_txt = "ecg_labels_txt", ecg_colname_num = "ecg_labels",  
                ecg_lbl_itos = ["1AVB", "RBBB", "LBBB", "SB", "ST", "AF"],
            ):
            ### - ecg - ###
            ecg_lbl_stoi = {s : i for i, s in enumerate(ecg_lbl_itos)}
            df[ecg_colname_txt] = df.apply(
                lambda row: ("1AVB " if row["1dAVb"] else "") + ("RBBB " if row["RBBB"] else "") + \
                ("LBBB " if row["LBBB"] else "") + ("SB " if row["SB"] else "") + ("ST " if row["ST"] else "") + ("AF " if row["AF"] else ""),
                axis = 1,
            )
            df[ecg_colname_txt] = df[ecg_colname_txt].apply(lambda x: [y for y in x.strip().split(" ") if y != ""])
            df[ecg_colname_num] = df[ecg_colname_txt].apply(lambda x: [ecg_lbl_stoi[y] for y in x if y in ecg_lbl_stoi.keys()])
            df.drop(["1dAVb", "RBBB", "LBBB", "SB", "ST", "AF"], axis = 1, inplace = True)
        
        reformat_predictions_code_15(exams_df, "ecg_labels_txt", "ecg_labels")
        
        df = exams_df
        df["ecg_labels_txt"] = df["ecg_labels_txt"].apply(lambda x: [""] if x == [] else x)
        df["data"] = [Path("code_15_" + str(df["exam_id"][i]) + ".npy") for i in range(len(df))]
        df["dataset"] = "code_15"
        
        # prepare raw data
        for f in os.listdir(data_path/Path("hdf5")): # load all hdf5
            print(f)
            with h5py.File(os.path.join(data_path/Path("hdf5"), f), "r") as h:
                print("Keys: %s" % h.keys())
                exam_id = h['exam_id'][()]
                sigbufs = h['tracings'][()]
                valid_ids = set(df["exam_id"])
                mask = np.isin(exam_id, list(valid_ids)) # Create a mask for valid exam_id elements
                # Filter exam_id and sigbufs using the mask
                filtered_exam_id = exam_id[mask]
                filtered_sigbufs = sigbufs[mask]
                exam_id = filtered_exam_id
                sigbufs = filtered_sigbufs
                # start_idxs = [np.where(np.sum(np.abs(sigbufs[i]), axis = 1) == 0.)[0] for i in range(len(sigbufs))] # discard zeros at beginning/end
                # start_idxs = [len(a) // 2 for a in start_idxs]
                
                for id in tqdm(range(len(list(sigbufs)))):
                    exam_id_valid = exam_id[id]
                    if np.all(sigbufs[id] == 0):
                        df = df[df["exam_id"] != exam_id_valid]
                        continue

                    exam_id_valid_set = set([exam_id_valid]) # Ensure exam_id_valid is a set for fast lookup
                    filtered_df = df[df["exam_id"].isin(exam_id_valid_set)]

                    data = resample_data(
                        sigbufs = sigbufs[id], # sigbufs[id][start_idxs[id]:-start_idxs[id] or None],
                        channel_stoi = channel_stoi,
                        channel_labels = channel_labels,
                        fs = fs,
                        target_fs = target_fs,
        ...
        ecg_lbl_itos.append("")
        stratified_ids = stratify(list(df["ecg_labels_txt"]), ecg_lbl_itos, [1./strat_folds] * strat_folds)
        ...
        # save
        save_dataset(df, ecg_lbl_itos, mean, std, target_root)
    else:
        df, ecg_lbl_itos, mean, std = load_dataset(target_root, df_mapped = False)

    return df, ecg_lbl_itos, mean, std


def prepare_data_code_15_chagas(
        data_path, 
        denoised = False, 
        target_fs = 100, 
        strat_folds = 10, 
    ...
    if (recreate_data is True):
        ecg_lbl_itos = ["1AVB", "RBBB", "LBBB", "SB", "ST", "AF"]
        chagas_lbl_itos = ["NOT_CHAGAS", "CHAGAS"]
        channel_labels = ["i", "ii", "iii", "avr", "avl", "avf", "v1", "v2", "v3", "v4", "v5", "v6"]
        fs = 400.

        # prepare df
        exams_df = pd.read_csv(data_path/"exams.csv")
        chagas_df = pd.read_csv(data_path/"code15_chagas_labels.csv")
        merged_df = exams_df.merge(chagas_df[['exam_id', 'chagas']], on = 'exam_id', how = 'left')
        merged_df = merged_df.dropna(subset=['chagas'])
        merged_df = merged_df.reset_index(drop = True)
        merged_df["sex"] = merged_df["is_male"].apply(lambda x: "male" if x else "female")
        merged_df.drop(["is_male"], axis = 1, inplace = True)
        merged_df["chagas"] = merged_df["chagas"].astype(bool)
        merged_df["not_chagas"] = ~merged_df["chagas"]

        def reformat_predictions_code_15(
                df, 
                ecg_colname_txt = "ecg_labels_txt", ecg_colname_num = "ecg_labels", 
                chagas_colname_txt = "chagas_labels_txt", chagas_colname_num = "chagas_labels", 
                ecg_lbl_itos = ["1AVB", "RBBB", "LBBB", "SB", "ST", "AF"], chagas_lbl_itos = ["NOT_CHAGAS", "CHAGAS"],
            ):
            ### - ecg - ###
            ecg_lbl_stoi = {s : i for i, s in enumerate(ecg_lbl_itos)}
            df[ecg_colname_txt] = df.apply(
            ...
            ### - chagas - ###
            chagas_lbl_stoi = {s : i for i, s in enumerate(chagas_lbl_itos)}
            df[chagas_colname_txt] = df.apply(
                lambda row: ("NOT_CHAGAS " if row["not_chagas"] else "") + ("CHAGAS " if row["chagas"] else ""),
                axis = 1,
            )
            df[chagas_colname_txt] = df[chagas_colname_txt].apply(lambda x: [y for y in x.strip().split(" ") if y != ""])
            df[chagas_colname_num] = df[chagas_colname_txt].apply(lambda x: [chagas_lbl_stoi[y] for y in x if y in chagas_lbl_stoi.keys()])
            df.drop(["not_chagas", "chagas"], axis = 1, inplace = True)
        
        reformat_predictions_code_15(merged_df, "ecg_labels_txt", "ecg_labels", "chagas_labels_txt", "chagas_labels")
        ...
        #     list(df.apply(lambda row: row["ecg_labels_txt"] + row["chagas_labels_txt"] + [row["sex"]], axis = 1)), 
        #     ecg_lbl_itos + chagas_lbl_itos + ["male", "female"], 
        #     [1./strat_folds] * strat_folds,
        # )

        stratified_ids = stratify(list(df.apply(lambda row: row["chagas_labels_txt"], axis = 1)), chagas_lbl_itos, [1./strat_folds] * strat_folds)
        ...
        # save
        save_dataset(df, chagas_lbl_itos, mean, std, target_root)
    else:
        df, chagas_lbl_itos, mean, std = load_dataset(target_root, df_mapped = False)

    return df, chagas_lbl_itos, mean, std


def prepare_data_leuven2024(
        data_path, 
        denoised = False, 
        target_fs = 100, 
        strat_folds = 10, 
    ...
    if (recreate_data is True):
        ...
        df["id"] = df.FileName
        df["data"] = df.FileName.apply(lambda x: x + ".npy")
        df["label_condition_txt"] = df.Group_2.apply(lambda x: [y for y in x.split(" ") if x != "NONE"])
        df["label_rhythm_txt"] = df.Group_3.apply(lambda x: x.split(" "))
        df["sex"] = df.Gender.apply(lambda x: x.lower())
        df.drop(["Gender", "Group_1", "Group_2", "Group_3", "FileName", "ID"], inplace = True, axis = 1)
        ...
        lbl_stoi["rhythm"] = {s : i for i, s in enumerate(lbl_itos["rhythm"])}
        df["label_rhythm"] = df["label_rhythm_txt"].apply(lambda x: [lbl_stoi["rhythm"][y] for y in x])
        df["dataset"] = "sinus_rhythm"

        for id, row in tqdm(list(df.iterrows())):
            fs = 500.

            df_tmp = pd.read_csv(data_path/("ECGData_sinus_rhythm")/(row["id"] + ".csv"))
            channel_labels = list(df_tmp.columns)
            sigbufs = np.array(df_tmp) * 0.001 # assuming data is given in muV
        ...
        stratified_ids = stratify(list(df["label_condition_txt"]), lbl_itos["condition"], [1./strat_folds] * strat_folds)
        # stratified_ids = stratify(list(df["label_rhythm_txt"]), lbl_itos["rhythm"], [1./strat_folds] * strat_folds)
        df["strat_fold"] = -1
        idxs = np.array(df.index.values)
        for i, split in enumerate(stratified_ids):
