...
class TimeseriesDatasetCrops(torch.utils.data.Dataset):
        ...
        if (memmap_filename is not None):
            ...
            if (annotation):
                memmap_meta_label = np.load(
                    self.memmap_meta_filename.parent/("_".join(self.memmap_meta_filename.stem.split("_")[:-1]) + "_label_meta.npz"), 
                    allow_pickle=True,
                )
    ...
    def _getitem(self, idx, start_idx_rel):
        ...
        if self.normalize_option == 0: # without normalization
            conver_to_tensor = ToTensor()
            sample = conver_to_tensor(sample)
        elif self.normalize_option == 1: # normalize sample
            mean_sample = sample[0].mean(axis = 0)
            std_sample = sample[0].std(axis = 0)
            norm = Normalize(mean_sample, std_sample)
            conver_to_tensor = ToTensor()
            sample = conver_to_tensor(norm(sample))
        elif self.normalize_option == 2: # normalize based on a given mean/std
