...
def interpolate(data, marker):
    ...
    data[data == marker] = np.interp(np.where(data == marker)[0], np.where(data != marker)[0], data[data != marker])
...
def refill(arr, center, radius, step):
    left_fill_values = arr[center - radius*step - radius : center - radius*step, :].copy()
    right_fill_values = arr[center + radius*step + 1 : center + radius*step + radius + 1, :].copy()
    arr[center - radius*step - radius : center - radius*step, :] = arr[center + radius*step + 1 : center + radius*step + radius + 1, :] = np.inf
...
class RandomResizedCrop(Transformation):
    ...
    def __init__(self, crop_ratio_range = [0.5, 1.0], output_size = 250):
        super(RandomResizedCrop, self).__init__(crop_ratio_range = crop_ratio_range, output_size = output_size)
    ...
    def __call__(self, sample):
        ...
        data, label = RandomCrop(int(crop_ratio * timesteps))(sample)  # apply random crop
        cropped_timesteps = data.shape[0]
        if output_timesteps >= cropped_timesteps:
            indices = np.sort(
                np.random.choice(np.arange(output_timesteps - 2) + 1, size = cropped_timesteps - 2, replace = False)
            )
            indices = np.concatenate([np.array([0]), indices, np.array([output_timesteps - 1])])
            ...
            indices = np.sort(np.random.choice(np.arange(cropped_timesteps), size = output_timesteps, replace = False))
            output = data[indices]
        return output, label
...
class DynamicTimeWarp(Transformation):
    ...
    def __init__(self, warps = 3, radius = 10, step = 2):
        super(DynamicTimeWarp, self).__init__(warps = warps, radius = radius, step = step)
        self.warps = warps
        self.radius = radius
        self.step = step
    ...
    def __call__(self, sample):
        ...
        for _ in range(self.warps):
            center = np.random.randint(self.min_center, timesteps - self.min_center - self.step)
...
class TDynamicTimeWarp(Transformation):
    ...
    def __init__(self, warps = 3, radius = 10, step = 2):
        super(TDynamicTimeWarp, self).__init__(warps = warps, radius = radius, step = step)
        self.warps = warps
        self.radius = radius
        self.step = step
        self.min_center = self.radius * (self.step + 1)
...
class TimeWarp(Transformation):
    ...
    def __init__(self, epsilon = 10, interpolation_kind = "linear", annotation = False):
        super(TimeWarp, self).__init__(epsilon = epsilon, interpolation_kind = interpolation_kind, annotation = annotation)
        self.scale = 1.
        self.loc = 0.
...
class ChannelResize(Transformation):
    ...
    def __call__(self, sample):
        data, label = sample
        timesteps, channels = data.shape
        resize_factors = np.exp(np.random.uniform(*self.log_magnitude_range, size = channels))
        resize_factors_same_shape = np.tile(resize_factors, timesteps).reshape(data.shape)
...
class DownSample(Transformation):
    ...
    def __call__(self, sample):
        ...
        data = data.copy()
        timesteps, channels = data.shape
        inpt_indices = np.random.choice(np.arange(timesteps - 2) + 1, size = int(self.downsample_ratio * timesteps), replace = False)
...
class ToTensor(Transformation):
    ...
    def __init__(self, transpose_data = True, transpose_label = False):
        super(ToTensor, self).__init__(transpose_data = transpose_data, transpose_label = transpose_label)
...
class TNormalize(Transformation):
    ...
    def __init__(self, stats_mean = None, stats_std = None, input = True, channels = []):
        super(TNormalize, self).__init__(stats_mean = stats_mean, stats_std = stats_std, input = input, channels = channels)
...
def baseline_wonder(ss_length = 250, fs = 100, C = 1, K = 50, df = 0.01):
    ...
    a_k = np.tile(np.random.uniform(0, 1, size = K), ss_length).reshape(K, ss_length, order = "F")
...
def noise_powerline(fs = 100, N = 1000, C = 1, fn = 50., K = 3, channels = 1):
    ...
    if (channels > 1):
        channel_gains = np.array([random.uniform(-1, 1) for _ in range(channels)])
        signal = signal * channel_gains[None]
    return signal
...
class BaselineWander(Transformation):
    ...
    def __call__(self, sample):
        ...
        data = data + noise_baseline_wander(
            fs = self.params["fs"], 
            N = len(data), 
            C = 0.05, 
            fc = self.params["fc"], 
            fdelta = self.params["fdelta"],
            channels = channels,
...
class BaselineShift(Transformation):
    ...
    def __init__(self, fs = 100, Cmax = 3, mean_segment_length = 3, max_segments_per_second = 0.3):
        super(BaselineShift, self).__init__(
            fs = fs, Cmax = Cmax, mean_segment_length = mean_segment_length, max_segments_per_second = max_segments_per_second
        )

    def __call__(self, sample):
        data, label = sample
        C = random.uniform(0, self.params["Cmax"])
        data = data + noise_baseline_shift(
            fs = self.params["fs"], 
            N = len(data), 
            C = C, 
...
class TBaselineShift(Transformation):
    ...
    def __call__(self, sample):
        data, label = sample     
        C = random.uniform(0,self.params["Cmax"])
        data = data + Tnoise_baseline_shift(
            fs = self.params["fs"], 
            N = len(data), 
            C = C, 
            mean_segment_length = self.params["mean_segment_length"],
